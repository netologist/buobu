...
export function ScheduledTransactionsDialog({ open, onOpenChange, rows, currency, swimlaneName }: Props) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[80vh] max-w-2xl overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Scheduled Transactions</DialogTitle>
        </DialogHeader>

        {rows.length === 0 ? (
          <p className="text-sm text-muted-foreground">No scheduled transactions for this swimlane.</p>
        ) : (
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="space-y-1">
                      ...
                      <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
                        ...
                        <span className="text-blue-600 dark:text-blue-400 font-medium">{tx.date ?? ""}</span>
