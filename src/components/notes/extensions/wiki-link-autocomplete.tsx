...
function renderWikiLinkItems(editor: Editor) {
  ...
  filteredNotes.forEach((note, idx) => {
    ...
    const tagsHtml = note.tags?.slice(0, 2).map((tag) => 
      `<span class="text-[10px] text-muted-foreground">#${tag}</span>`
    ).join("") || "";
    
    el.innerHTML = `
      <div class="flex h-8 w-8 shrink-0 items-center justify-center rounded-md border bg-background">
